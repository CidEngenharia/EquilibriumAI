...
const IkigaiDiagram = ({ isDarkMode }) => {
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      ...
      {/* Diagrama SVG */}
      <div className={`relative rounded-2xl p-8 border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100 shadow-xl'}`}>
      ...
      {activeCategory && category && (
        <div className={`rounded-2xl border p-6 animate-fade-in ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100 shadow-lg'}`}>
          ...
          <div className="flex gap-2 mb-4">
            <input
              ...
              className={`flex-1 px-4 py-2.5 rounded-xl border text-sm outline-none transition-all ${
                isDarkMode
                  ? 'bg-slate-900 border-slate-700 text-white placeholder:text-slate-500 focus:border-brand-teal/50'
